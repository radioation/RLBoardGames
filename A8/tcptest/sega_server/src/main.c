    ...
    { 
        ...
        if( mode == 0 ) {
            ...
            if(buttons & BUTTON_START && buttons_prev == 0x00) { 
                VDP_drawText("send SH", cursor_x, cursor_y); cursor_y++;
                NET_sendMessage("SH"); 
                NET_flushBuffers();
            }
            if(buttons & BUTTON_A && buttons_prev == 0x00) { 
                if( NET_TXReady() ) {
                    VDP_drawText("send BA", cursor_x, cursor_y); cursor_y++;
                    NET_sendByte('B');
                    NET_sendByte('A');
                    NET_flushBuffers();
                }
            ...
            if(buttons & BUTTON_B && buttons_prev == 0x00) { 
                VDP_drawText("send BUTTON B", cursor_x, cursor_y); cursor_y++;
                NET_sendMessage("BUTTON B");
                NET_flushBuffers();
            }
            if(buttons & BUTTON_C && buttons_prev == 0x00) { 
                VDP_drawText("send BUTTON C", cursor_x, cursor_y); cursor_y++;
                NET_sendMessage("BUTTON C");
                NET_flushBuffers();
            }
        }
